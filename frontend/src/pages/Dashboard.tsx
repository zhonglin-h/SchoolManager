...
function AttendanceCard({ event, onGuestRegistered }: { event: AttendanceSummaryResponse; onGuestRegistered?: (displayName: string, id: number, personType: 'STUDENT' | 'TEACHER') => void }) {
  ...
  const [quickAdd, setQuickAdd] = useState<{ email: string; initialName?: string; eventTitle: string; mode: 'student' | 'teacher' } | null>(null)
                  ...
                  <td className="py-1">
                    {!entry.registered && (
                      <div className="flex gap-1">
                        <button
                          onClick={() => setQuickAdd({ email: entry.email, eventTitle: event.eventTitle, mode: 'student' })}
                          className="text-xs bg-blue-50 text-blue-600 hover:bg-blue-100 px-2 py-0.5 rounded"
                        >
                          + Student
                        </button>
                        <button
                          onClick={() => setQuickAdd({ email: entry.email, eventTitle: event.eventTitle, mode: 'teacher' })}
                    ...
                    <tr key={guest.googleUserId ?? i} className="border-b last:border-0">
                      ...
                      <td className="py-1">
                        {!guest.personType && (
                          <div className="flex gap-1">
                            <button
                              onClick={() => setQuickAdd({ email: '', initialName: guest.displayName ?? '', eventTitle: event.eventTitle, mode: 'student' })}
                              className="text-xs bg-blue-50 text-blue-600 hover:bg-blue-100 px-2 py-0.5 rounded"
                            >
                              + Student
                            </button>
                            <button
                              onClick={() => setQuickAdd({ email: '', initialName: guest.displayName ?? '', eventTitle: event.eventTitle, mode: 'teacher' })}
                              className="text-xs bg-purple-50 text-purple-600 hover:bg-purple-100 px-2 py-0.5 rounded"
                            >
                              + Teacher
                            </button>
